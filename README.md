...
### مفيش SSH ولا Terminal؟ (استضافة سحابية بسيطة)

بعض باقات cPanel الرخيصة توفر File Manager وMySQL Databases وCron Jobs بس، بدون SSH ولا Terminal — السكريبت أعلاه مش هيشتغل لأنه محتاج شل. الحل: **معالج تثبيت عبر المتصفح** (`/install`) موجود جاهز داخل التطبيق نفسه، مبني بحيث يعمل بالكامل بدون سطر أوامر واحد. التفاصيل والمخاطر الأمنية موضحة في [docs/08 § 8.2.5](./docs/08-System-Architecture-Security-Deployment.md#825-النشر-بدون-sshterminal-استضافة-سحابية-بسيطة).

باختصار: تحمّل حزمة جاهزة فيها `vendor/` مبني مسبقًا (لأن Composer مش هيشتغل بدون شل) و`.env` بمفتاح تشفير جاهز و`INSTALL-TOKEN.txt` برمز عشوائي خاص بيك، ترفعها بـ File Manager، توجّه Document Root لمجلد `public/` بداخلها، وتفتح `https://دومينك/install` من المتصفح — المعالج هيسألك عن رمز التثبيت وبيانات قاعدة البيانات، ويعمل الباقي (Migrations + بيانات النظام + حساب Super Admin) لوحده. بعد النجاح يقفل نفسه نهائيًا تلقائيًا.
